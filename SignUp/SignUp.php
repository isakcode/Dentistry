<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Регистрация</title>
    <link rel="stylesheet" href="css/SignUp.css" />
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@100;400;500;600;800&display=swap" rel="stylesheet">
</head>
<?php 
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        $username = filter_var(trim($_POST["name"]), FILTER_SANITIZE_STRING); 
        $email = filter_var(trim($_POST["email"]), FILTER_SANITIZE_STRING); 
        $telNum = filter_var(trim($_POST["telNum"]),FILTER_SANITIZE_STRING);
        $pass = filter_var(trim($_POST["pass"]), FILTER_SANITIZE_STRING);
        $pass = md5($pass."anjfifna02932"); 
        $mysql = new mysqli('localhost','root','',"member'sdb"); 
        $mysql->query("INSERT INTO `memberstable`(`Name`,`Email`,`Tel`,`Password`) VALUES('$username', '$email','$telNum','$pass')"); 
        $mysql->close();
    }
?>
<body>
    <header>
        <div class="InfoHead">
            <div>
                <a href="../index.php" class="LogoName">
                    Premium
                </a>
            </div>
            <div class="InfoClinic">
                <div class="InfoItemHead ItemHeadPhone">
                    <img src="../css/img/icons/phone.svg" class="PhoneHead">
                    <div class="ItemHeadText">
                        <div class="ItemHeadFunct">
                            Телефон
                        </div>
                        <div class="ItemHeadData">
                            (237) 681-812-255
                        </div>
                    </div>
                </div>
                <div class="InfoItemHead ItemHeadTime">
                    <img src="../css/img/icons/clock.svg" class="WorkTime">
                    <div class="ItemHeadText">
                        <div class="ItemHeadFunct">
                            Время работы
                        </div>
                        <div class="ItemHeadData">
                            09:00 - 20:00 Everyday
                        </div>
                    </div>
                </div>
                <div class="InfoItemHead ItemHeadLocation">
                    <img src="../css/img/icons/location.svg" class="Location">
                    <div class="ItemHeadText">
                        <div class="ItemHeadFunct">
                            Адрес
                        </div>
                        <div class="ItemHeadData">
                            Конаева 21
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </header>
    <main>
        <div class="FormBlock">
            <div class="FormName">
                Регистрация
            </div>
            <form action = "#" method="post" class="Form" name="form" id="Form">
                <div class="FormItem">
                    <label for="Name">
                        Имя
                    </label>
                    <input type='text' id="Name" class="name" name="name">
                </div>
                <div class="FormItem">
                    <label for="Email">
                        Эл. почта
                    </label>
                    <input type="email" id="Email" class="email" name="email">
                </div>
                <div class="FormItem">
                    <label for="TelNum">
                        Номер
                    </label>
                    <input type="tel" id="TelNum" class="TelNum" name="telNum">
                </div>
                <div class="FormItem">
                    <label for="Pass">
                        Пароль
                    </label>
                    <input type='password' id="Pass" class="name" name="pass">
                </div>
                <button type="submit" class="BtnSignUp">
                    Зарегистрироваться
                </button>
            </form>
            <a href="../SignIn/SignIn.php" class="LinkSignIn">У вас есть аккаунт?</a>
        </div>
    </main>
</body>

</html>